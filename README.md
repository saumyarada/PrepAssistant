...
profile, gives you an overview of your progress, and helps you practice
smarter with AI-generated hints, solution fixes, and next-problem
suggestions.
...
PrepAssistant sits on top of your LeetCode activity and closes that loop
with a lightweight AI layer.
...
3. User pastes in a problem they're working on
4. User can ask for a Socratic-style hint, or submit a broken solution for
   an AI-assisted fix (diagnosis + minimal patch, not a rewrite)
5. App suggests the next problem to practice based on weak topics and
   difficulty progression

## Feature List (current)

1. LeetCode profile sync + overview dashboard (solved by difficulty/topic)
2. AI hints on a problem (given problem + user's current code)
3. AI solution fixing (given problem + code + error/failing case)
4. Next-problem suggestions (based on topic/difficulty gaps)
5. Simple static frontend to exercise all of the above
...
| AI            | Gemini API                     | Hints, solution fixing, suggestions               |
...
(`services/leetcode_service.py`) so a breaking change is a one-file fix.
...
|---|---|---|
| GET | `/api/leetcode/{username}` | Fetch (and cache) a user's LeetCode profile stats |
...
- **No authentication yet.** The simple frontend requires manually
  copying a `user_id` from the database to use the AI features. Real
  auth (Supabase Auth / Clerk) is planned before deployment.
...
- [x] Phase 4 (partial): Simple static frontend to exercise the API
